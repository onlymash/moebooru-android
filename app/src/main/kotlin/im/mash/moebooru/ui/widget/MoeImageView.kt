...
class MoeImageView : AppCompatImageView {
    ...
    private val mClickRunnable = Runnable {
        if (mClickListener != null) {
            mClickListener!!.onClick(this@MoeImageView)
        }
    }
    ...
    private val mGestureListener = object : GestureDetector.SimpleOnGestureListener() {
        ...
        override fun onLongPress(e: MotionEvent) {
            if (mLongClick != null) {
                mLongClick!!.onLongClick(this@MoeImageView)
            }
        }
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        width = when {
            p.width == ViewGroup.LayoutParams.MATCH_PARENT -> if (widthMode == View.MeasureSpec.UNSPECIFIED) {
                drawableW
            } else {
                pWidth
            }
        ...
        height = when {
            p.height == ViewGroup.LayoutParams.MATCH_PARENT -> if (heightMode == View.MeasureSpec.UNSPECIFIED) {
                drawableH
            } else {
                pHeight
            }
    ...
    private inner class Transform internal constructor() : Runnable {
        ...
        internal lateinit var clipCalculate: ClipCalculate
        ...
        override fun run() {
            ...
            if (mClipScroller.computeScrollOffset() || mClip != null) {
                ...
                mTmpMatrix.setScale(sx, sy, (mImgRect.left + mImgRect.right) / 2, clipCalculate.calculateTop())
    ...
    /**
     * 在 MoeImageView 内部还没有图片的时候同样可以调用该方法
     *
     *
     * 此时并不会播放动画，当给 MoeImageView 设置图片后会自动播放动画。
     *
     *
    ...
    fun animaFrom(info: Info) {
        if (isInit) {
            ...
            if (info.mWidgetRect.width() < info.mImgRect.width() || info.mWidgetRect.height() < info.mImgRect.height()) {
                ...
                val c = if (info.mScaleType == ImageView.ScaleType.FIT_START) START() else if (info.mScaleType == ImageView.ScaleType.FIT_END) END() else OTHER()
